...
describe('KaraokeText', () => {
  ...
  it('커스텀 색상이 올바르게 적용되는지 확인', () => {
    render(<KaraokeText text="Colored Text" progress={50} color="#333333" activeColor="#FF0000" />)

    const container = screen.getByText('Colored Text', {
      selector: '.karaoke-text__base',
    }).parentElement
  ...
  it('커스텀 폰트 크기가 올바르게 적용되는지 확인', () => {
    ...
    const container = screen.getByText('Big Text', {
      selector: '.karaoke-text__base',
    }).parentElement
